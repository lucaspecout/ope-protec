...
const FLUX_SERVICES = [
  ...
  { key: 'meteo_forets_isere',     label: 'Météo des forêts · Isère',  icon: '🌲', category: 'Météo',         interval: 3600,  metric: (d) => `Danger ${d.forecasts?.[0]?.danger || d.danger || 'indisponible'}` },
  ...
  { key: 'feux_foret_isere',       label: 'Feux de forêt Isère',       icon: '🔥', category: 'Risques',       interval: 180,   metric: (d) => `${d.fires_total ?? 0} foyer(s) FeuxDeForet.fr < 2j` },
  ...
  { key: 'cols_alpins_isere',      label: 'Cols alpins Isère',         icon: '⛰️', category: 'Transport',     interval: 21600, metric: (d) => `${d.cols_total ?? 0} cols · ${d.dangereux_total ?? 0} à surveiller` },
  { key: 'anfr_isere',             label: 'ANFR · Antennes mobiles',   icon: '📡', category: 'Télécom',       interval: 21600, metric: (d) => Number.isFinite(Number(d.supports_total)) ? `${Number(d.supports_total)} support(s) mobile recensés` : 'Données ANFR temporairement indisponibles' },
  ...
  { key: 'finess_isere',           label: 'FINESS · Établissements santé', icon: '🏥', category: 'Santé',    interval: 604800, metric: (d) => `${d.resources_total ?? 0} établissement(s)` },
  { key: 'geodae_isere',           label: "Geo'DAE - Defibrillateurs", icon: 'DAE', category: 'Santé', interval: 604800, metric: (d) => `${d.resources_total ?? 0} DAE - ${d.available_24h_total ?? 0} 24h/24` },
];
...
function setMapFeedback(message = '', isError = false) {
  ...
  target.className = `map-feedback-bar ${isError ? 'error' : 'muted'}`;
}
...
async function startIsoZoneMode(mode) {
  const nextMode = mapIsoMode === mode ? null : mode;
  if (nextMode && !leafletMap) {
    try {
      setMapFeedback('Chargement de la carte avant activation de l outil iso...');
      if (typeof window.L !== 'undefined') initMap();
      if (!leafletMap) await ensureMapReady();
    } catch (error) {
      setMapFeedback(`Outil iso indisponible: ${sanitizeErrorMessage(error.message)}`, true);
      return;
    }
  }
  if (mapAddPointMode) {
...
const _NOTIF_SERVICES = [
  ...
  { key: 'aprr_isere',            label: 'APRR/AREA · Autoroutes Isère', icon: '🛣️', cat: 'Transport' },
  { key: 'vinci_autoroutes',      label: 'Vinci Autoroutes',           icon: '🛣️', cat: 'Transport' },
