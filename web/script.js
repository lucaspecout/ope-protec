...
function ensureSituationContainer() {
  const panel = document.getElementById('situation-panel');
  if (!panel) return null;

  panel.querySelectorAll('.situation-toolbar, .situation-top-grid, .situation-middle-grid').forEach((node) => node.remove());

  let container = document.getElementById('situation-content');
  if (!container) {
    container = document.createElement('div');
    container.id = 'situation-content';
    container.className = 'situation-content';
...
function renderSituationOverview() {
  const target = ensureSituationContainer();
  if (!target) return;
