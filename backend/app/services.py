...
def _fetch_georisques_v2_collection(
    ...
    query = {"pageSize": page_size, "pageNumber": 0}
    if extra_query:
        query.update(extra_query)

    has_codes_insee = bool(query.get("codesInsee"))
    if not has_codes_insee and "departement" not in query:
        query["departement"] = departement

    first_page = _http_get_json(
        f"https://georisques.gouv.fr/api/v2/{endpoint}?{urlencode(query, doseq=True)}",
        headers=headers,
    )
